...
from dogepartylib.lib.messages.versions import enhanced_send
from dogepartylib.lib.messages import order
from dogepartylib.lib.messages import dogepay
...
from dogepartylib.lib.messages import destroy
...
from dogepartylib.lib.messages import sweep
from dogepartylib.lib.messages import dispenser

API_TABLES = ['assets', 'balances', 'credits', 'debits', 'bets', 'bet_matches',
              'broadcasts', 'dogepays', 'burns', 'cancels', 'destructions',
...
API_TRANSACTIONS = ['bet', 'broadcast', 'dogepay', 'burn', 'cancel', 'destroy',
...
def check_backend_state():
    """Checks blocktime of last block to see if {} Core is running behind.""".format(config.DOGE_NAME)
    block_count = backend.getblockcount()
    block_hash = backend.getblockhash(block_count)
    #cblock = backend.getblock(block_hash)
    cblock = backend.getjsonblock(block_hash)
    ...
    if time_behind > 60 * 60 * 2:   # Two hours.
        raise BackendError('Dogecoind is running about {} hours behind.'.format(round(time_behind / 3600)))
...
def check_database_state(db, blockcount):
    """Checks {} database to see if is caught up with backend.""".format(config.XDP_NAME)
    if util.CURRENT_BLOCK_INDEX + 1 < blockcount:
        raise DatabaseError('{} database is behind backend.'.format(config.XDP_NAME))
    ...
    def run(self):
        ...
        while self.stop_event.is_set() != True:
            try:
                ...
                # Check that the database has caught up with dogecoind.
    ...
    def run(self):
        ...
        def get_supply(asset):
            if asset == config.DOGE:
                return  backend.get_doge_supply(normalize=False)
            elif asset == config.XDP:
                return util.xdp_supply(self.db)
            else:
        ...
        @dispatcher.add_method
        def get_xdp_supply():
            logger.warning("Deprecated method: `get_xdp_supply`")
            return util.xdp_supply(self.db)
        ...
        def get_asset_info(assets):
            ...
            for asset in assets:
                ...
                if asset in [config.DOGE, config.XDP]:
                    if asset == config.DOGE:
                        supply = backend.get_doge_supply(normalize=False)
                    else:
                        supply = util.xdp_supply(self.db)
        ...
        def get_running_info():
            ...
            return {
                ...
                'dogecoin_block_count': latestBlockIndex,
        ...
        def get_element_counts():
            ...
            for element in ['transactions', 'blocks', 'debits', 'credits', 'balances', 'sends', 'orders',
                'order_matches', 'dogepays', 'issuances', 'broadcasts', 'bets', 'bet_matches', 'dividends',
        ...
        def get_tx_info(tx_hex, block_index=None):
            ...
            source, destination, doge_amount, fee, data, extra = blocks.get_tx_info(tx_hex, block_index=block_index)
            return source, destination, doge_amount, fee, util.hexlify(data) if data else ''
