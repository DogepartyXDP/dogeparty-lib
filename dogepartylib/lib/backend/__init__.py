...
from dogepartylib.lib.backend import addrindexrs
...
def BACKEND():
    mdl = sys.modules['dogepartylib.lib.backend.{}'.format(config.BACKEND_NAME)]
    mdl.init()
    return mdl
...
def get_doge_supply(normalize=False):
    """returns the total supply of {} (based on what Dogecoin Core says the current block height is)""".format(config.DOGE)
