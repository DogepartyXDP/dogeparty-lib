...
from dogepartylib.lib import config, util, address
...
def rpc_call(payload):
    """Calls to dogecoin core and returns the response"""
    ...
    if 'error' not in response_json.keys() or response_json['error'] == None:
        ...
        raise BackendRPCError('{} Is `txindex` enabled in {} Core?'.format(response_json['error'], config.DOGE_NAME))
    elif response_json['error']['code'] in [-28, -8, -2]:
        # “Verifying blocks...” or “Block height out of range” or “The network does not appear to fully agree!“
        logger.debug('Backend not ready. Sleeping for ten seconds.')
        # If Dogecoin Core takes more than `sys.getrecursionlimit() * 10 = 9970`
...
def rpc_batch(request_list):
    ...
    def make_call(chunk):
        #send a list of requests to dogecoind to be executed
        #note that this is list executed serially, in the same thread in dogecoind
        #e.g. see: https://github.com/dogecoin/dogecoin/blob/master/src/rpcserver.cpp#L939
