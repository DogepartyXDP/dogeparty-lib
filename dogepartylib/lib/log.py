...
class ModuleLoggingFilter(logging.Filter):
    ...
    module level logging filter (NodeJS-style), ie:
        filters="*,-dogepartylib.lib,dogepartylib.lib.api"

        will log:
         - dogepartycli.server
         - dogepartylib.lib.api

        but will not log:
         - dogepartylib.lib
         - dogepartylib.lib.backend.indexd
    """
    ...
    def ismatch(cls, record, name):
        ...
        Determine if the specified record matches the name, in the same way as original logging.Filter does, ie:
            'dogepartylib.lib' will match 'dogepartylib.lib.check'
        """
        nlen = len(name)
        if nlen == 0:
            return True
...
def log (db, command, category, bindings):
    ...
    if command == 'update':
        ...
        if category == 'credits':
            ...
            logger.info('Order: {} ordered {} for {} in {} blocks, with a provided fee of {:.8f} {} and a required fee of {:.8f} {} ({}) [{}]'.format(bindings['source'], output(bindings['give_quantity'], bindings['give_asset']), output(bindings['get_quantity'], bindings['get_asset']), bindings['expiration'], bindings['fee_provided'] / config.UNIT, config.DOGE, bindings['fee_required'] / config.UNIT, config.DOGE, bindings['tx_hash'], bindings['status']))
        ...
        elif category == 'dogepays':
            logger.info('{} Payment: {} paid {} to {} for order match {} ({}) [{}]'.format(config.DOGE, bindings['source'], output(bindings['doge_amount'], config.DOGE), bindings['destination'], bindings['order_match_id'], bindings['tx_hash'], bindings['status']))
            ...
            logger.info('Bet: {} against {}, by {}, on {}'.format(output(bindings['wager_quantity'], config.XDP), output(bindings['counterwager_quantity'], config.XDP), bindings['source'], bindings['feed_address']))
            ...
            logger.info('Bet Match: {} for {} against {} for {} on {} at {}{} ({}) [{}]'.format(util.BET_TYPE_NAME[bindings['tx0_bet_type']], output(bindings['forward_quantity'], config.XDP), util.BET_TYPE_NAME[bindings['tx1_bet_type']], output(bindings['backward_quantity'], config.XDP), bindings['feed_address'], isodt(bindings['deadline']), placeholder, bindings['id'], bindings['status']))
            ...
            logger.info('Burn: {} burned {} for {} ({}) [{}]'.format(bindings['source'], output(bindings['burned'], config.DOGE), output(bindings['earned'], config.XDP), bindings['tx_hash'], bindings['status']))

        elif category == 'cancels':
            logger.info('Cancel: {} ({}) [{}]'.format(bindings['offer_hash'], bindings['tx_hash'], bindings['status']))

        elif category == 'rps':
            log_message = 'RPS: {} opens game with {} possible moves and a wager of {}'.format(bindings['source'], bindings['possible_moves'], output(bindings['wager'], config.XDP))
            logger.info(log_message)

        elif category == 'rps_matches':
            log_message = 'RPS Match: {} is playing a {}-moves game with {} with a wager of {} ({}) [{}]'.format(bindings['tx0_address'], bindings['possible_moves'], bindings['tx1_address'], output(bindings['wager'], config.XDP), bindings['id'], bindings['status'])
            logger.info(log_message)
            ...
            if bindings['status'] == 'valid':
                ...
                log_message = 'RPS Resolved: {} is playing {} on a {}-moves game with {} with a wager of {} ({}) [{}]'.format(rps_match['tx0_address'], bindings['move'], rps_match['possible_moves'], rps_match['tx1_address'], output(rps_match['wager'], config.XDP), rps_match['id'], rps_match['status'])
            else:
            ...
            if bindings['bet_match_type_id'] == cfd_type_id:
                if bindings['settled']:
                    logger.info('Bet Match Settled: {} credited to the bull, {} credited to the bear, and {} credited to the feed address ({})'.format(output(bindings['bull_credit'], config.XDP), output(bindings['bear_credit'], config.XDP), output(bindings['fee'], config.XDP), bindings['bet_match_id']))
                else:
                    logger.info('Bet Match Force‐Liquidated: {} credited to the bull, {} credited to the bear, and {} credited to the feed address ({})'.format(output(bindings['bull_credit'], config.XDP), output(bindings['bear_credit'], config.XDP), output(bindings['fee'], config.XDP), bindings['bet_match_id']))

            elif bindings['bet_match_type_id'] == equal_type_id:
                logger.info('Bet Match Settled: {} won the pot of {}; {} credited to the feed address ({})'.format(bindings['winner'], output(bindings['escrow_less_fee'], config.XDP), output(bindings['fee'], config.XDP), bindings['bet_match_id']))
            ...
            if bindings['status'] == 0:
                ...
                logger.info('Dispenser: {} (empty address) opened a {} for asset {} with {} balance, giving {} {} for each {} {}'.format(bindings['source'], bindings['asset'], escrow_quantity, give_quantity, bindings['asset'], each_price, currency))
            ...
            if (dispenser["oracle_address"] != None) and util.enabled('oracle_dispensers'):
                tx_doge_amount = get_tx_info(cursor, bindings['tx_hash'])/config.UNIT
                ...
                fiatpaid = round(tx_doge_amount*oracle_last_price,2)
                
                logger.info('Dispense: {} from {} to {} for {:.8f} {} ({} {}) ({})'.format(output(bindings['dispense_quantity'], bindings['asset']), bindings['source'], bindings['destination'], tx_doge_amount, config.DOGE, fiatpaid, oracle_fiat_label, bindings['tx_hash']))
            else:
...
def get_asset_info(cursor, asset):
    if asset == config.DOGE or asset == config.XDP:
...
def get_tx_info(cursor, tx_hash):
    ...
    return transaction["doge_amount"]
