...
from dogepartylib.lib import (config, exceptions, util, message_type)
...
def validate (db, source, quantity_per_unit, asset, dividend_asset, block_index):
    ...
    if asset == config.DOGE:
        problems.append('cannot pay dividends to holders of {}'.format(config.DOGE))
    if asset == config.XDP:
        if (not block_index >= 317500) or block_index >= 320000 or config.TESTNET or config.REGTEST:   # Protocol change.
            problems.append('cannot pay dividends to holders of {}'.format(config.XDP))
    ...
    # Examine asset.
    try:
        divisible = util.is_divisible(db, asset, not util.enabled("util_divisible_fix",block_index))
    except AssetError:
    ...
    try:
        dividend_divisible = util.is_divisible(db, dividend_asset, not util.enabled("util_divisible_fix",block_index))
    except AssetError:
    ...
    for holder in holders:
        ...
        if dividend_asset == config.DOGE and dividend_quantity < config.DEFAULT_MULTISIG_DUST_SIZE: continue    # A bit hackish.
    ...
    fee = 0
    if not problems and dividend_asset != config.DOGE:
        ...
        if fee:
            balances = list(cursor.execute('''SELECT * FROM balances WHERE (address = ? AND asset = ?)''', (source, config.XDP)))
            if not balances or balances[0]['quantity'] < fee:
                problems.append('insufficient funds ({})'.format(config.XDP))

    if not problems and dividend_asset == config.XDP:
...
def parse (db, tx, message):
    ...
    if dividend_asset == config.DOGE:
        status = 'invalid: cannot pay {} dividends within protocol'.format(config.DOGE)
    ...
    if status == 'valid':
        ...
        if tx['block_index'] >= 330000 or config.TESTNET or config.REGTEST: # Protocol change.
            util.debit(db, tx['source'], config.XDP, fee, action='dividend fee', event=tx['tx_hash'])
