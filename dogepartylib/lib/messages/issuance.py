...
from dogepartylib.lib import (config, util, exceptions, util, message_type)

FORMAT_1 = '>QQ?'
LENGTH_1 = 8 + 8 + 1
FORMAT_2 = '>QQ???If'
LENGTH_2 = 8 + 8 + 1 + 1 + 1 + 4 + 4
SUBASSET_FORMAT = '>QQ??B'
SUBASSET_FORMAT_LENGTH = 8 + 8 + 1 + 1 + 1
ID = 20
SUBASSET_ID = 21
...
def validate (db, source, destination, asset, quantity, divisible, lock, callable_, call_date, call_price, description, subasset_parent, subasset_longname, block_index):
    problems = []
    fee = 0

    if asset in (config.DOGE, config.XDP):
        problems.append('cannot issue {} or {}'.format(config.DOGE, config.XDP))
    ...
    if quantity or (block_index >= 315000 or config.TESTNET or config.REGTEST):   # Protocol change.
        if not reissuance or (block_index < 310000 and not config.TESTNET and not config.REGTEST):  # Pay fee only upon first issuance. (Protocol change.)
            ...
            cursor.execute('''SELECT * FROM balances \
                              WHERE (address = ? AND asset = ?)''', (source, config.XDP))
    ...
    return call_date, call_price, problems, fee, description, divisible, lock, reissuance, reissued_asset_longname


def compose (db, source, transfer_destination, asset, quantity, divisible, lock, description):
    ...
    call_date, call_price, problems, fee, description, divisible, lock, reissuance, reissued_asset_longname = validate(db, source, transfer_destination, asset, quantity, divisible, lock, callable_, call_date, call_price, description, subasset_parent, subasset_longname, util.CURRENT_BLOCK_INDEX)
    ...
    if subasset_longname is None or reissuance:
        ...
        data += struct.pack(curr_format, asset_id, quantity, 1 if divisible else 0, 1 if lock else 0, 1 if callable_ else 0,
        ...
        data += struct.pack(curr_format, asset_id, quantity, 1 if divisible else 0, 1 if lock else 0, compacted_subasset_length, compacted_subasset_longname, description.encode('utf-8'))
...
def parse (db, tx, message, message_type_id):
    ...
    try:
        ...
        if message_type_id == SUBASSET_ID:
            ...
            lock, callable_, call_date, call_price, description = False, False, 0, 0.0, ''
        try:
        ...
        asset, quantity, divisible, lock, callable_, call_date, call_price, description = None, None, None, None, None, None, None, None
    ...
    fee = 0
    if status == 'valid':
        call_date, call_price, problems, fee, description, divisible, lock, reissuance, reissued_asset_longname = validate(db, tx['source'], tx['destination'], asset, quantity, divisible, lock, callable_, call_date, call_price, description, subasset_parent, subasset_longname, block_index=tx['block_index'])
