...
def cancel_rps (db, rps, status, block_index):
    ...
    util.credit(db, rps['source'], config.XDP, rps['wager'], action='recredit wager', event=rps['tx_hash'])
...
def update_rps_match_status (db, rps_match, status, block_index):
    ...
    if status in ['expired', 'concluded: tie']:
        ...
        util.credit(db, rps_match['tx0_address'], config.XDP,
                    rps_match['wager'], action='recredit wager', event=rps_match['id'])
        # Recredit tx1 address.
        util.credit(db, rps_match['tx1_address'], config.XDP,
                    rps_match['wager'], action='recredit wager', event=rps_match['id'])
        ...
        util.credit(db, winner, config.XDP,
...
def parse(db, tx, message):
    ...
    if status == 'open':
        util.debit(db, tx['source'], config.XDP, wager, action="open RPS", event=tx['tx_hash'])
    ...
    for rps_match in cursor.fetchall():
        ...
        if new_rps_match_status == 'expired':
            ...
            for rps in matched_rps:
                ...
                # Re-debit XDP refund by close_rps_match.
                util.debit(db, rps['source'], config.XDP, rps['wager'], action='reopen RPS after matching expiration', event=rps_match['id'])
                # Rematch
