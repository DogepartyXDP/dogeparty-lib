...
from dogepartylib.lib import (config, util, exceptions, util, message_type, address)
...
def unpack(db, message, block_index):
    try:
        ...
        if asset == config.DOGE:
            raise exceptions.AssetNameError('{} not allowed'.format(config.DOGE))
...
def validate (db, source, destination, asset, quantity, memo_bytes, block_index):
    ...
    if asset == config.DOGE: problems.append('cannot send {}'.format(config.DOGE))
...
def compose (db, source, destination, asset, quantity, memo, memo_is_hex):
    ...
    cursor.close()
    # return an empty array as the second argument because we don't need to send DOGE dust to the recipient
    return (source, [], data)
