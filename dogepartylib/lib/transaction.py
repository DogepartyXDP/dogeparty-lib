"""
Construct and serialize the Dogecoin transactions that are Dogeparty transactions.
...
from dogepartylib.lib.transaction_helper import serializer, p2sh_encoding
...
def construct_coin_selection(encoding, data_array, source, allow_unconfirmed_inputs, unspent_tx_hash, custom_inputs,
    ...
    # Array of UTXOs, as retrieved by listunspent function from dogecoind
    if custom_inputs:
    ...
    for coin in use_inputs:
        ...
        inputs.append(coin)
        doge_in += round(coin['amount'] * config.UNIT)

        # If exact fee is specified, use that. Otherwise, calculate size of tx
        # and base fee on that (plus provide a minimum fee for selling DOGE).
        ...
        # Check if good.
        doge_out = destination_doge_out + data_doge_out
        change_quantity = doge_in - (doge_out + final_fee)
        logger.debug('Size: {} Fee: {:.8f} Change quantity: {:.8f} DOGE'.format(size, final_fee / config.UNIT, change_quantity / config.UNIT))
    ...
    if not sufficient_funds:
        ...
        # quantities.
        doge_out = destination_doge_out + data_doge_out
        total_doge_out = doge_out + max(change_quantity, 0) + final_fee
        raise exceptions.BalanceError('Insufficient {} at address {}. (Need approximately {} {}.) To spend unconfirmed coins, use the flag `--unconfirmed`. (Unconfirmed coins cannot be spent from multi‐sig addresses.)'.format(config.DOGE, source, total_doge_out / config.UNIT, config.DOGE))
    ...
    return inputs, change_quantity, doge_in, final_fee
...
def construct (db, tx_info, encoding='auto',
    ...
    destination_doge_out = sum([value for address, value in destination_outputs])
    ...
    logger.getChild('p2shdebug').debug('data_doge_out=%s (data_value=%d len(data_array)=%d)' % (data_doge_out, data_value, len(data_array)))
    ...
    if encoding == 'p2sh':
        ...
        size_for_fee, datatx_necessary_fee, data_value, data_doge_out = p2sh_encoding.calculate_outputs(destination_outputs, data_array, fee_per_kb, exact_fee)
    ...
    if not (encoding == 'p2sh' and p2sh_pretx_txid):
        inputs, change_quantity, n_doge_in, n_final_fee = construct_coin_selection(
            encoding, data_array,
            source, allow_unconfirmed_inputs, unspent_tx_hash, custom_inputs,
            fee_per_kb, estimate_fee_per_kb, estimate_fee_per_kb_nblocks,
            exact_fee, size_for_fee, fee_provided, destination_doge_out, data_doge_out,
            regular_dust_size, disable_utxo_locks
        )
        doge_in = n_doge_in
    ...
    except exceptions.DOGEOnlyError:
        # Skip DOGE‐only transactions.
        if extended_tx_info:
            return {
                'doge_in': doge_in,
                'doge_out': destination_doge_out + data_doge_out,
                'doge_change': change_quantity,
                'doge_fee': final_fee,
                'tx_hex': unsigned_tx_hex,
            }
        logger.getChild('p2shdebug').debug('DOGE-ONLY')
