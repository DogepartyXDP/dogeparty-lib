...
from dogepartylib.lib import api, config, util, exceptions, blocks, check, backend, database, transaction, script
...
def initialise_config(database_file=None, log_file=None, api_log_file=None,
    ...
    # Log directory
    log_dir = appdirs.user_log_dir(appauthor=config.XDP_NAME, appname=config.APP_NAME)
    ...
    # Backend Core RPC user (Dogecoin Core)
    if backend_user:
        config.BACKEND_USER = backend_user
    else:
        config.BACKEND_USER = 'dogecoinrpc'

    # Backend Core RPC password (Dogecoin Core)
    if backend_password:
...
def kickstart(db, dogecoind_dir):
    blocks.kickstart(db, dogecoind_dir=dogecoind_dir)
